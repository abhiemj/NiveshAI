...
# First-run convenience: auto-create config.ini from the template if missing,
# so new users can clone, run, and just paste their OpenAI key (no manual copy).
_CONFIG_FILE = os.path.join(CONFIG_DIR, "config.ini")
_CONFIG_EXAMPLE = os.path.join(CONFIG_DIR, "config.ini.example")
if not os.path.exists(_CONFIG_FILE) and os.path.exists(_CONFIG_EXAMPLE):
    import shutil
    shutil.copyfile(_CONFIG_EXAMPLE, _CONFIG_FILE)
    logger.info(f"Created {_CONFIG_FILE} from template. Add your OpenAI API key to start generating reports.")

app = FastAPI(title="Nivesh AI Equity Research", version="1.0.0")
...
async def run_analysis(req: AnalysisRequest, request: Request, background_tasks: BackgroundTasks):
    ...
    # Sanitize inputs: strip whitespace (a trailing space in the ticker would
    # otherwise create output folders/links with spaces and 404 the report).
    req.ticker = req.ticker.strip().upper()
    req.company_name = req.company_name.strip()
    req.peers = [p.strip().upper() for p in req.peers if p and p.strip()]
    if not req.ticker:
