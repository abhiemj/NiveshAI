...
def main():
    ...
    parser.add_argument("--research-source", type=str, default="Nivesh AI Equity Research · Built on FinRobot (Apache 2.0)", help="Source of the research.")
    parser.add_argument("--data-source-text", type=str, default="Company Filings, Yahoo Finance, Nivesh AI Estimates", help="Text for data sources.")
    ...
    try:
        ...
        # FMP is no longer used (data comes from Yahoo Finance), so the key is optional.
        try:
    ...
    # Currency symbol (auto-detected from the data source; defaults to $)
    cur = auto_fetched_metrics.get('currency_symbol', '$') or '$'

    # Propagate the currency symbol to the formatter modules. Some tables/figures
    # are formatted here (before the renderers run and set their own currency), so
    # we set the module-level symbol directly to keep everything consistent.
    import modules.html_renderer as _html_renderer_mod
    import modules.html_template_professional as _html_prof_mod
    _html_renderer_mod._CUR = cur
    _html_prof_mod._CUR = cur

    # Apply the logic for each metric
    share_price = get_value(args.share_price, 'share_price', 0.0, lambda x: f"{cur}{x:.2f}")
    target_price = get_value(args.target_price, 'target_price', 0.0, lambda x: f"{cur}{x:.2f}")
    rating = get_value(args.rating, 'rating', "N/A")
    market_cap = get_value(args.market_cap, 'market_cap', 0.0, lambda x: f"{cur}{x:,.2f}B")
    ...
    if args.enable_valuation_analysis:
        ...
        try:
            ...
            financial_data_for_valuation = {
                'analysis': analysis_df,
                'current_price': float(_re.sub(r'[^0-9.]', '', share_price)) if isinstance(share_price, str) else share_price,
            ...
            valuation_engine = ValuationEngine(financial_data_for_valuation, peer_data_for_valuation, currency_symbol=cur)
