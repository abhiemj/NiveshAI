...
# Display currency symbol; set per-report from data['currency_symbol'] at render
# time. Defaults to "$" so US tickers are unchanged.
_CUR = "$"


def _set_currency(data: dict):
    global _CUR
...
def _format_value(val):
    ...
    if isinstance(val, str):
        ...
        try:
            ...
            if abs(num_val) >= 1_000_000_000:
                return f"{_CUR}{num_val/1_000_000_000:,.1f}B"
            elif abs(num_val) >= 1_000_000:
                return f"{_CUR}{num_val/1_000_000:,.1f}M"
    ...
    if isinstance(val, (int, float)):
        ...
        if abs(val) >= 1_000_000_000:
            return f"{_CUR}{val/1_000_000_000:,.1f}B"
        if abs(val) >= 1_000_000:
            return f"{_CUR}{val/1_000_000:,.1f}M"
        if abs(val) >= 1_000:
            return f"{val:,.0f}"
    ...
    if synthesis and synthesis.get('target_price'):
        ...
        <div style="background:#f5f7fa; border-top:2px solid #002855; padding:10px 16px; display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:8px; margin-bottom:12px;">
            <div>
                ...
                <p style="color:#002855; font-size:18px; font-weight:700;">{_CUR}{synth_target:.2f}</p>
            </div>
            <div>
                <p style="color:#555555; font-size:9px; text-transform:uppercase; letter-spacing:0.04em; margin-bottom:2px;">Valuation Range</p>
                <p style="color:#1a1a1a; font-size:13px; font-weight:600;">{_CUR}{synth_range[0]:.2f} - {_CUR}{synth_range[1]:.2f}</p>
            </div>
            <div>
...
def format_enhanced_charts_html(charts_data, report_data=None):
    ...
    ma_val = f'SMA 50: {_CUR}{sma50:.2f}' if sma50 else ''
    if sma200:
        ma_val += f' | SMA 200: {_CUR}{sma200:.2f}'
