...
class ValuationEngine:
    ...
    def __init__(self, financial_data: Dict, peer_data: Dict = None, currency_symbol: str = "$"):
        """
        初始化估值引擎
        ...
        Args:
            ...
            peer_data: 同行公司数据
            currency_symbol: 显示用货币符号 (e.g. "₹" for Indian listings)
        """
    ...
    def explain_valuation_differences(self) -> str:
        ...
        explanation_parts = [
            ...
            f"The valuation analysis reveals a range of {cur}{lowest[1]:.2f} to {cur}{highest[1]:.2f}, "
            f"representing a {diff_pct:.1f}% spread between methods.\n",
            f"\n### Key Differences:\n",
            f"- **{lowest[0]}** provides the most conservative estimate at {cur}{lowest[1]:.2f}",
            f"- **{highest[0]}** suggests the highest value at {cur}{highest[1]:.2f}",
        ]
        ...
        explanation_parts.append(f"\n### Synthesis:")
        explanation_parts.append(f"Weighted average target price: {cur}{synthesis['target_price']:.2f}")
