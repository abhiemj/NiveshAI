...
def get_enhanced_company_news(ticker: str, api_key: str, days_back: int = 5, 
    ...
    """
    try:
        # News is now sourced from Yahoo Finance (see market_data_api.get_company_news)
        from .market_data_api import get_company_news

        logger.info(f"Fetching enhanced news for {ticker} (Yahoo Finance)...")
        raw_news = get_company_news(ticker, api_key=api_key, days_back=days_back, limit=limit)
