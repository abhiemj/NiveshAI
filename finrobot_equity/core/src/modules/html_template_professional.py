...
# Display currency symbol; set per-report from report_data['currency_symbol'].
# Defaults to "$" so US tickers are unchanged.
_CUR = "$"
    ...
    <div class="max-w-4xl mx-auto px-6 pb-12" style="margin-top: -1rem;">
        ...
        <section class="mt-12" id="disclaimer" style="background: #f8fafc; margin-left: -1.5rem; margin-right: -1.5rem; padding: 1.5rem; border-radius: 0.75rem;">
            <div class="flex items-center gap-2 mb-2">
                <span style="background: linear-gradient(90deg, #6366f1, #8b5cf6); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-weight: 600; font-size: 0.75rem;">Powered by Nivesh AI · built on FinRobot</span>
...
def _format_figure_value(value) -> str:
    ...
    abs_num = abs(num)
    if abs_num >= 1e12:
        return f"{_CUR}{num/1e12:,.2f}T"
    elif abs_num >= 1e9:
        return f"{_CUR}{num/1e9:,.2f}B"
    elif abs_num >= 1e6:
        return f"{_CUR}{num/1e6:,.1f}M"
    elif abs_num >= 1e3:
        return f"{_CUR}{num/1e3:,.1f}K"
    elif abs_num == 0:
        return f"{_CUR}0"
    else:
        return f"{num:,.2f}"
        ...
        for metric, ci in sensitivity_data['confidence_intervals'].items():
            if ci and isinstance(ci, dict):
                ...
                if isinstance(low_val, (int, float)):
                    low_val = f"{_CUR}{low_val/1e9:.1f}B" if low_val > 1e9 else f"{_CUR}{low_val/1e6:.1f}M"
                if isinstance(high_val, (int, float)):
                    high_val = f"{_CUR}{high_val/1e9:.1f}B" if high_val > 1e9 else f"{_CUR}{high_val/1e6:.1f}M"
...
def format_valuation_breakdown_html(valuation_data: dict) -> str:
    ...
    for m in methods:
        target = f"{_CUR}{m['target_price']:.2f}"
        low = f"{_CUR}{m['low_estimate']:.2f}"
        high = f"{_CUR}{m['high_estimate']:.2f}"
    ...
    if synthesis and synthesis.get('target_price'):
        ...
        <div style="background:linear-gradient(135deg, #eef2ff 0%, #f5f3ff 100%); border-radius:0.75rem;
            ...
            <div>
                ...
                <p style="color:#0f172a; font-size:1.5rem; font-weight:700;">{_CUR}{synth_target:.2f}</p>
            </div>
            <div>
                ...
                <p style="color:#0f172a; font-size:1rem; font-weight:600;">{_CUR}{synth_range[0]:.2f} - {_CUR}{synth_range[1]:.2f}</p>
            </div>
            <div>
...
def format_advanced_charts_html_professional(data: dict) -> str:
    ...
    ma_detail = ''
    if sma50:
        ma_detail += f'SMA 50: <strong>{_CUR}{sma50:.2f}</strong>'
    if sma200:
        ma_detail += f' &nbsp;|&nbsp; SMA 200: <strong>{_CUR}{sma200:.2f}</strong>'
    if not ma_detail:
...
def _derive_rating(share_price, target_price, api_rating: str) -> str:
    ...
    """
    try:
        price = float(_re.sub(r'[^0-9.]', '', str(share_price)))
        target = float(_re.sub(r'[^0-9.]', '', str(target_price)))
        if price <= 0:
...
def render_professional_html_report(data: dict) -> str:
    ...
    # Set the display currency for all module-level formatters (e.g. ₹ for .NS)
    global _CUR
    _CUR = data.get('currency_symbol', '$') or '$'
    ...
    report_data = {
        ...
        'research_source': data.get('research_source', 'Nivesh AI · Built on FinRobot (Apache 2.0)'),
